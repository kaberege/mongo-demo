...
export const userRegister = async (req, res, next) => {
    ...
    try {
        ...
        if (collides) {
            const error = new Error("Unique context collision: Email already exists.");
...
export const userLogin = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) {
            const error = new Error("Invalid authentication parameters mapped.");
        ...
        const accessToken = jwt.sign({ userId: user._id.toString(), role: user.role }, JWT_SECRET || "secretetoken", { expiresIn: "15m" });
        const refreshToken = jwt.sign({ userId: user._id.toString() }, JWT_REFRESH_SECRET || "refreshtoken", { expiresIn: "7d" });
...
export const tokenRefreshRotation = async (req, res, next) => {
    const cookies = req.headers.cookie;
    const cookieMap = Object.fromEntries(cookies?.split("; ").map((c) => c.split("=")) || []);
    const refreshToken = cookieMap["refreshToken"];
    if (!refreshToken) {
        const error = new Error("Session trace lost. Login sequence forced.");
        error.statusCode = 401;
        return next(error);
    }
    const secretRefreshTokenkey = JWT_REFRESH_SECRET || "";
    if (!secretRefreshTokenkey) {
        const error = new Error("Refresh Token Secret configuration missing.");
        error.statusCode = 401;
        return next(error);
    }
    try {
        const decodedToken = jwt.verify(refreshToken, secretRefreshTokenkey);
        if (!decodedToken) {
        ...
        if (!user) {
            const error = new Error("Target core identity mapping resolved out of system context bounds.");
            error.statusCode = 401;
            throw error;
        }
        const newAccessToken = jwt.sign({ userId: user._id.toString(), role: user.role }, JWT_SECRET || "secretetoken", { expiresIn: "15m" });
        res.status(200).json({ token: newAccessToken });
    }
    catch (err) {
        const error = new Error("Session signature verification failed.");
...
export const resetPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) {
            const error = new Error("Verification string expired or context is invalid.");
...
export const updateProfile = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) {
            const error = new Error("Profile modification target missing.");
            error.statusCode = 404;
            throw error;
        }
        if (name)
...
export const adminModifyRole = async (req, res, next) => {
    const { role } = req.body;
    try {
        const targetAccount = await User.findById(req.params.userId);
        if (!targetAccount) {
            const error = new Error("Target subject user mismatch.");
            error.statusCode = 404;
            throw error;
        }
